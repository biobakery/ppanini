...
filename="stool/ecoli/norm_stool_ecoli_table.csv"
...
clust = cutreeStatic(sampleTree, cutHeight=200000, minSize=10) #custom
table(clust)
keepSamples = (clust==1)
...
net = blockwiseModules(data.matrix(datExpr), power=16, TOMType="unsigned", minModuleSize=30, reassignThreshold=0, mergeCutHeight=0.25, numericLabels=TRUE, pamRespectsDendro=FALSE, saveTOMs=TRUE, saveTOMFileBase= paste(filename, "PFTOM", sep=""), verbose=3)
...
TOM = TOMsimilarityFromExpr(data.matrix(datExpr), power =16)
