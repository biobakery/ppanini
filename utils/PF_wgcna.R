...
##Loading Data
getwd();
workingDir="~/Desktop/ppanini_data/gene_network";
setwd(workingDir);
library(WGCNA);
...
clust = cutreeStatic(sampleTree, cutHeight=100000, minSize=10) #custom
table(clust)
keepSamples = (clust==1)
...
net = blockwiseModules(data.matrix(datExpr), power=12, TOMType="unsigned", minModuleSize=30, reassignThreshold=0, mergeCutHeight=0.25, numericLabels=TRUE, pamRespectsDendro=FALSE, saveTOMs=TRUE, saveTOMFileBase= paste(filename, "PFTOM", sep=""), verbose=3)
